...
void graphNP(vector<vector<int>>& g, int n, double p) {
...
int main() {
  ...
  double probability = 0.008;

  std::map<int, int> results;
  int cases = 100;
  for (int i = 0; i < cases; ++i) {
    graphNP(g, n, probability);
    ...
    if (!tree.has_value()) {
      ...
      // std::cout << "n = " << real_tree.size() << "\n";
      results[span.max_degree] = results[span.max_degree] + 1;
      // std::cout << "max degree k = " << span.max_degree << "\n";
    }
  }
